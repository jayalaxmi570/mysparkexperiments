...
object KafkaProducerWeblogs {
  def main(args: Array[String]) {
    val spark = SparkSession.builder.master("local[*]").appName("KafkaProducerWeblogs").config("spark.logConf", "true").config("spark.logLevel", "ERROR").getOrCreate()
    val ssc = new StreamingContext(spark.sparkContext, Seconds(10))
    //val sc = spark.sparkContext
    val path = "C:\\work\\datasets\\weblogs"
    val topic = "indpak"
